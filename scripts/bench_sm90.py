...
from flash_attn_interface import _flash_attn_forward as _flash_attn_fwd, _flash_attn_backward as _flash_attn_bwd
...
def bench_fwd(batch, seqlen, nheads, hdim, causal, tile_m=None, tile_n=None,
    ...
    try:
        out = _flash_attn_fwd(q, k, v, **kwargs)
...
def bench_bwd(batch, seqlen, nheads, hdim, causal, warmup=5, rep=30, hdim_v=None, **bwd_kwargs):
    ...
    try:
        out = _flash_attn_fwd(q, k, v, softmax_scale=softmax_scale, causal=causal)
    except Exception as e:
        return None, None, str(e)[:80]
