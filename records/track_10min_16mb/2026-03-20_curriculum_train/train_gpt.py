...
class Hyperparameters:
    ...
    curriculum_min_seq_len = int(os.environ.get("CURRICULUM_MIN_SEQ_LEN", str(max(128, train_seq_len // 8))))
    curriculum_steps = int(os.environ.get("CURRICULUM_STEPS", str(iterations)))
    ...
    beta1 = float(os.environ.get("BETA1", 0.7))
...
class DistributedTokenLoader:
    ...
    def next_batch(self, global_tokens: int, seq_len: int, grad_accum_steps: int) -> tuple[Tensor, Tensor]:
        local_token_budget = global_tokens // (self.world_size * grad_accum_steps)
        local_batch_seqs = local_token_budget // seq_len
        if local_batch_seqs <= 0:
            raise ValueError(
                f"TRAIN_BATCH_TOKENS={global_tokens} is too small for seq_len={seq_len}, "
...
def build_curriculum_seq_lens(min_seq_len: int, max_seq_len: int) -> list[int]:
    if max_seq_len <= 0:
        raise ValueError(f"TRAIN_SEQ_LEN must be positive, got {max_seq_len}")
    curr = max(1, min(min_seq_len, max_seq_len))
    seq_lens = [curr]
    while seq_lens[-1] < max_seq_len:
        seq_lens.append(min(seq_lens[-1] * 2, max_seq_len))
    return seq_lens


def get_train_seq_len_for_step(step: int, curriculum_seq_lens: list[int], curriculum_steps: int) -> int:
    if not curriculum_seq_lens:
        raise ValueError("curriculum_seq_lens must not be empty")
    if len(curriculum_seq_lens) == 1 or curriculum_steps <= 0:
        return curriculum_seq_lens[-1]
    stage_idx = min((max(step, 0) * len(curriculum_seq_lens)) // curriculum_steps, len(curriculum_seq_lens) - 1)
    return curriculum_seq_lens[stage_idx]
...
def main() -> None:
    ...
    local_train_token_budget = args.train_batch_tokens // (world_size * grad_accum_steps)
    if local_train_token_budget < args.train_seq_len:
        raise ValueError(
            "TRAIN_BATCH_TOKENS must provide at least one full training sequence per rank; "
            f"got TRAIN_BATCH_TOKENS={args.train_batch_tokens}, WORLD_SIZE={world_size}, "
            f"GRAD_ACCUM_STEPS={grad_accum_steps}, TRAIN_SEQ_LEN={args.train_seq_len}"
        )
    curriculum_seq_lens = build_curriculum_seq_lens(args.curriculum_min_seq_len, args.train_seq_len)
    ...
    log0(
        f"train_curriculum:min_seq_len:{curriculum_seq_lens[0]} "
        f"stages:{','.join(str(seq_len) for seq_len in curriculum_seq_lens)} "
    ...
    if args.warmup_steps > 0:
        ...
        for warmup_step in range(args.warmup_steps):
            curr_seq_len = get_train_seq_len_for_step(warmup_step, curriculum_seq_lens, args.curriculum_steps)
            zero_grad_all()
            ...
            if args.warmup_steps <= 20 or (warmup_step + 1) % 10 == 0 or warmup_step + 1 == args.warmup_steps:
                log0(f"warmup_step:{warmup_step + 1}/{args.warmup_steps} train_seq_len:{curr_seq_len}")
    ...
    swa_count = 0
    curr_train_seq_len = get_train_seq_len_for_step(0, curriculum_seq_lens, args.curriculum_steps)
    prev_train_seq_len: int | None = None
    ...
    while True:
        ...
        curr_train_seq_len = get_train_seq_len_for_step(step, curriculum_seq_lens, args.curriculum_steps)
        if curr_train_seq_len != prev_train_seq_len:
            log0(f"curriculum_step:{step}/{args.iterations} train_seq_len:{curr_train_seq_len}")
            prev_train_seq_len = curr_train_seq_len
        zero_grad_all()
        train_loss = torch.zeros((), device=device)
        for micro_step in range(grad_accum_steps):
            if distributed:
                model.require_backward_grad_sync = micro_step == grad_accum_steps - 1
            x, y = train_loader.next_batch(args.train_batch_tokens, curr_train_seq_len, grad_accum_steps)
            with torch.autocast(device_type="cuda", dtype=torch.bfloat16, enabled=True):
                loss = model(x, y)
        ...
        if should_log_train:
            log0(
                ...
                f"train_seq_len:{curr_train_seq_len} "
